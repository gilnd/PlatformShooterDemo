...
if(currentWeapon[? "cartridge_ammo"] >= (currentWeapon[? "cartridge"]/2)){
	...
	bullet.image_angle = dir;
	bullet.image_speed = global.GAME_FPS;
	bullet.sprite_index = sRP46BulletNerf;
	bullet.alarm[1] = irandom(3)+1;
	bullet.col1 = c_red;
	bullet.col2 = make_color_rgb(244,111,116);
	audio_play_sound(choose(sndRedHipsterFire1, sndRedHipsterFire2, sndRedHipsterFire3, sndRedHipsterFire4), 1, false);
	// IMPORTANT
	bullet.damage = irandom_range(4, 6);
...
repeat(irandom(3)+1){
	...
	P.image_blend = choose(c_red, c_white);
	P.decrease = 0.9;
}
//Smoke
repeat(irandom(2)){
...
repeat(irandom_range(3,6)) {
	...
	B.colour1 = c_red;
	B.colour2 = make_color_rgb(244,111,116);
...
if(choose(true, false, false, false)){
	...
	C.col1 = c_red;
	C.col2 = make_color_rgb(244,111,116);
